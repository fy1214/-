...
func CreateUser(c *gin.Context) {
	...
	//密码加密
	model.SetPassword(&member, member.Password)

	//数据添加至数据库
	err = model.CreateAMember(&member)
	if err != nil {
		c.JSON(http.StatusOK, model.ResponseMeta{Code: model.UnknownError})
		return
	}
...
func UpdateUser(c *gin.Context) {
	...
	//从请求中拿出数据
	var member1 model.UpdateMemberRequest
	c.BindJSON(&member1)
	...
	//查找需要修改的用户
	member2, err := model.GetAMember(id)
	if err != nil {
		c.JSON(http.StatusOK, model.ResponseMeta{Code: model.UserNotExisted})
		return
	}
	if member2.Deleted {
		c.JSON(http.StatusOK, model.ResponseMeta{Code: model.UserHasDeleted})
		return
	}

	//修改昵称
	model.UpdateAMember(member2, "name", nickname)
	c.JSON(http.StatusOK, model.ResponseMeta{Code: model.OK})
	return
}
...
func DeleteUser(c *gin.Context) {
	...
	//从请求中拿出数据
	var member1 model.DeleteMemberRequest
	c.BindJSON(&member1)
	id, _ := strconv.ParseUint(member1.UserID, 10, 64)
...
func GetAUser(c *gin.Context) {
	...
	//从请求中拿出数据
	var member1 model.GetMemberRequest
	c.BindJSON(&member1)
	id, _ := strconv.ParseUint(member1.UserID, 10, 64)
...
func GetUserList(c *gin.Context) {
	...
	//从请求中拿出数据
	var listRequest model.GetMemberListRequest
	c.BindJSON(&listRequest)
	offset := listRequest.Offset
	limit := listRequest.Limit

	memberList, err := model.GetMemberList(limit, offset)
	if err != nil {
		c.JSON(http.StatusOK, model.ResponseMeta{Code: model.UnknownError})
		return
	}
